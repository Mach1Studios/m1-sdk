...
public:
    ...
    void updateAngles() {
            float distanceYaw = std::abs(targetYaw - currentYaw);
            if (((distanceYaw) > 0.6) && (distanceYaw < 360)) {
        ...
        float distancePitch = std::abs(targetPitch - currentPitch);
        if (((distancePitch) > 0.6) && (distancePitch < 360)) {
        ...
        float distanceRoll = std::abs(targetRoll - currentRoll);
        if (((distanceRoll) > 0.6) && (distanceRoll < 360)) {
...
//
    ...
    static float mmap(float value, float inputMin, float inputMax, float outputMin, float outputMax,
                      bool clamp = false) {

        if (fabs(inputMin - inputMax) < __FLT_EPSILON__) {
            return outputMin;
        } else {
            float outVal = ((value - inputMin) / (inputMax - inputMin) * (outputMax - outputMin) +
                            outputMin);
...
//
    ...
    std::vector<float> fourChannelAlgorithm(float Yaw, float Pitch, float Roll) {
...
//
    ...
    std::vector<float> fourPairsAlgorithm(float Yaw, float Pitch, float Roll) {
...
//
    ...
    std::vector<float> eightChannelsIsotropicAlgorithm(float Yaw, float Pitch, float Roll) {
        ...
        mPoint faceVector21 = faceVector1.getRotated(simulationAngles[0] + 90,
                                                     mPoint(cos(
                                                             mDegToRad(simulationAngles[1] - 90)),
                                                            sin(mDegToRad(simulationAngles[1] -
                                                                          90))).normalize());
...
//
    ...
    std::vector<float> eightChannelsAlgorithm(float Yaw, float Pitch, float Roll) {
...
//
    ...
    std::vector<float> eightPairsAlgorithm(float Yaw, float Pitch, float Roll) {
