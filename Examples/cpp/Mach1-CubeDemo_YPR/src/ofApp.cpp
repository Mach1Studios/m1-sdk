...
void ofApp::setup(){
    ...
    decoder.setup("/dev/cu.Mach1-02-DevB", 115200);
...
//    };
    ...
    decoder.gotNewValues = [&](float A, float B, float C) {
        a = A;
        b = B;
        c = C;
    };
...
void ofApp::update(){
    ...
    for (auto &i : tests) {
        ...
        float pitchAngle = mmap(b, 0, 360, -180.0, 180.0, true);
        float pitchAngleClamp = ofClamp(pitchAngle, -90.0, 90.0);
        
        float rollAngle = mmap(c, 0, 360, -180.0, 180.0, true);
        float rollAngleClamp = ofClamp(rollAngle, -90.0, 90.0);
        
        serialAngleUpdate(a, pitchAngleClamp, rollAngleClamp);
    }
}
...
//--------------------------------------------------------------
void ofApp::serialAngleUpdate(float serialY, float serialP, float serialR){
