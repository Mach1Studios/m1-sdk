                     ...
                     double streamTime, RtAudioStreamStatus status, void *userData )
{
	...
	if (inputBuffers.size() != m1transcode.getInputNumChannels()) {
		inputBuffers.resize(m1transcode.getInputNumChannels());
		for (c = 0; c < m1transcode.getInputNumChannels(); c++) {
			inputBuffers[c].resize(nBufferFrames);
		}
	}
	if (transcodedBuffers.size() != m1transcode.getOutputNumChannels()) {
		transcodedBuffers.resize(m1transcode.getOutputNumChannels());
		for (c = 0; c < m1transcode.getOutputNumChannels(); c++) {
