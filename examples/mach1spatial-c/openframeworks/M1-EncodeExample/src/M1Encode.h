...
public:
    ...
    typedef float PPoint[7];
    
    struct GeneratePointResult {
        PPoint ppoints[7];
        int pointsCount;
...
//                            resultingPoints.ppoints[1][2] = center[2] - sin((sRotationInRadians)) * sSpread; // Z
        ...
        for (int i = 0; i < resultingPoints.pointsCount; i++) {
            ...
            resultingPoints.ppoints[i][0] = resultingPoints.ppoints[i][0] / 2 + 0.5;
            resultingPoints.ppoints[i][1] = resultingPoints.ppoints[i][1] / 2 + 0.5;
            resultingPoints.ppoints[i][2] = resultingPoints.ppoints[i][2] / 2 + 0.5;
            ...
            if (outputMode == OUTPUT_4CH) {
                float gains[4];
                processGains4Channels(resultingPoints.ppoints[i][0],
                                      resultingPoints.ppoints[i][2], gains);
            ...
            if (outputMode == OUTPUT_8CH) {
                float gains[8];
                processGains8Channels(resultingPoints.ppoints[i][0],
                                      resultingPoints.ppoints[i][2],
                                      resultingPoints.ppoints[i][1], gains);
                for (int j = 0; j < outputChannelCount; j++) {
                    resultingPoints.gains[i][j] = gains[j];
                }
            }
    ...
    void processGains4Channels (float x, float y,
                                float (&result)[4]) {
    ...
    void processGains8Channels (float x, float y, float z,
                                float (&result)[8]) {
    ...
    enum InputMode {INPUT_MONO, INPUT_STEREO, INPUT_QUAD, INPUT_AFORMAT, INPUT_BFORMAT};
    enum OutputMode {OUTPUT_4CH, OUTPUT_8CH};
