...
inline mPoint M1DSPAlgorithms::mPoint::operator+( const mPoint& pnt ) const {
            return mPoint( x+pnt.x, y+pnt.y, z+pnt.z );
        }
        
        
inline mPoint M1DSPAlgorithms::mPoint::operator*( const float f ) const {
            return mPoint( x*f, y*f, z*f );
        }
        
        
inline mPoint M1DSPAlgorithms::mPoint::operator*( const mPoint& vec ) const {
...
inline mPoint& M1DSPAlgorithms::mPoint::rotate( float angle, const mPoint& axis ) {
...
inline mPoint& M1DSPAlgorithms::mPoint::normalize() {
...
inline mPoint M1DSPAlgorithms::mPoint::getNormalized() const {
            float length = (float)sqrt(x*x + y*y + z*z);
            if( length > 0 ) {
...
inline mPoint M1DSPAlgorithms::mPoint::getRotated( float angle, const mPoint& axis ) const {
            mPoint ax = axis.getNormalized();
            float a = (float)(angle*DEG_TO_RAD);
            float sina = sin( a );
            float cosa = cos( a );
...
static float M1DSPAlgorithms::mDegToRad(float degrees) {
...
static float M1DSPAlgorithms::mmap(float value, float inputMin, float inputMax, float outputMin, float outputMax, bool clamp = false) {
...
static float M1DSPAlgorithms::clamp(float a, float min, float max )
    {
        return (a < min) ? min : ((a > max) ? max : a);
    }

static float M1DSPAlgorithms::alignAngle(float a, float min = -180, float max = 180)
    {
        while (a < min) a += 360;
        while (a > max) a -= 360;
...
void M1DSPAlgorithms::updateAngles() {
    ...
    std::vector<float> M1DSPAlgorithms::fourChannelAlgorithm(float Yaw, float Pitch, float Roll, bool smoothAngles = false) {
    ...
    std::vector<float> M1DSPAlgorithms::fourPairsAlgorithm(float Yaw, float Pitch, float Roll, bool smoothAngles = false) {
    ...
    std::vector<float> M1DSPAlgorithms::eightChannelsIsotropicAlgorithm(float Yaw, float Pitch, float Roll, bool smoothAngles = false) {
    ...
    std::vector<float> M1DSPAlgorithms::eightChannelsAlgorithm(float Yaw, float Pitch, float Roll, bool smoothAngles = false) {
    ...
    std::vector<float> M1DSPAlgorithms::eightPairsAlgorithm(float Yaw, float Pitch, float Roll, bool smoothAngles = false) {
