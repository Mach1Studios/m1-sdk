...
class ThreeChannelTest: public BaseAudioTest {
public:
    ThreeChannelTest() {
        for (int i = 0; i < 3; i++){
            playersLeft[0].load("3ChannelAudio/1/1.wav"); playersLeft[0].setLoop(true); playersLeft[0].play();
            playersLeft[1].load("3ChannelAudio/1/2.wav"); playersLeft[1].setLoop(true); playersLeft[1].play();
            playersLeft[2].load("3ChannelAudio/1/3.wav"); playersLeft[2].setLoop(true); playersLeft[2].play();
            playersLeft[i].setPan(-1);
            playersRight[0].load("3ChannelAudio/1/1.wav"); playersRight[0].setLoop(true); playersRight[0].play();
            playersRight[1].load("3ChannelAudio/1/2.wav"); playersRight[1].setLoop(true); playersRight[1].play();
            playersRight[2].load("3ChannelAudio/1/3.wav"); playersRight[2].setLoop(true); playersRight[2].play();
    ...
    void draw() {
        ...
        ofDrawSphere(-200, -20, 100, volumes[4] * 15 + 5);
        ofSetColor(0, 0, 200);
        ofDrawSphere(-200, 20, 100, volumes[5] * 15 + 5);
        ...
        ofSetColor(255);
        ofDrawBitmapString("3", -200, 0, 100);
    ...
    void drawOverlay() {
        ofDrawBitmapStringHighlight("Three channel test", 20, ofGetHeight() - 70);
    ...
    ofSoundPlayer playersLeft[3];
    ofSoundPlayer playersRight[3];
    ...
    std::vector<float> audioMixAlgorithm(float X, float Y, float Z) {
        ...
        result[1] = coefficients[2]; //   right
        result[2] = coefficients[1]; // 2 left
        result[3] = coefficients[0]; //   right
        result[4] = coefficients[2]; // 3 left
        result[5] = coefficients[1]; //   right
