...
const eightChannelsIsotropicAlgorithm = __webpack_require__(0).eightChannelsIsotropicAlgorithm;
...
const audioFiles8 = (function () {
    ...
    for (let side of ['left', 'right'])
    {
        ...
        for (let posH of ['l', 'h'])
        {
            ...
            {
                for (let posLR of (posFB == 'f' ? ['l', 'r'] : ['l', 'r']))
                {
                    audio[side].push({
                        src: "./audio/" + i + "_" + side[0] + ".wav",
    ...
    console.log("left: ", audio.left);
    console.log("right: ", audio.right);
    ...
    let player = window.player = videojs('videojs-panorama-player', {}, function() {
        ...
        function updateVolumes() {
            if (thus.getChild('Canvas'))
            {
                ...
                // fucking magic
                sound.updateVolumes(alpha, -theta);
            }
        }
...
function Sound(audioFiles, player) {
    ...
    function setVolumeOnSide(volumes, side) {
        for (let i in volumes[side])
        {
            ...
            {
                if (volumes[side][i] < 0 || volumes[side][i] > 1 || isNaN(volumes[side][i]))
                {
                    console.warn('Volumes error(out of range) volumes[' + side + '][' + i + ']:' + volumes[side][i]);
    ...
    {
        let vols = eightChannelsIsotropicAlgorithm(yaw, pitch, roll);
