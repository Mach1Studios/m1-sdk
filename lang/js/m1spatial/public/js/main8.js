...
function eightChannelsIsotropicAlgorithm(yaw, pitch, roll) {
    let simulationAngles = new Point(yaw, pitch, roll);

    let faceVector1 = new Point(Math.cos(Point.degToRad(simulationAngles.at(1))),
        Math.sin(Point.degToRad(simulationAngles.at(1)))).normalize();


    let faceVector2 = faceVector1.getRotated(simulationAngles.at(0),
        new Point(Math.cos(Point.degToRad(simulationAngles.at(1) - 90)),
            Math.sin(Point.degToRad(simulationAngles.at(1) - 90))).normalize());


    let faceVector21 = faceVector1.getRotated(simulationAngles.at(0) + 90,
        new Point(Math.cos(Point.degToRad(simulationAngles.at(1) - 90)),
            Math.sin(Point.degToRad(simulationAngles.at(1) - 90))).normalize());

    let faceVectorLeft = faceVector21.getRotated(-simulationAngles.at(2) - 90, faceVector2);
    let faceVectorRight = faceVector21.getRotated(-simulationAngles.at(2) + 90, faceVector2);

	// ???
    // let time = ofGetElapsedTimef() * 5;

    let faceVectorOffsetted = Point.minus(new Point(Math.cos(Point.degToRad(simulationAngles.at(1))),
            Math.sin(Point.degToRad(simulationAngles.at(1)))).normalize().rotate(
            simulationAngles.at(0) + 10,
            new Point(Math.cos(Point.degToRad(simulationAngles.at(1) - 90)),
                Math.sin(Point.degToRad(simulationAngles.at(1) - 90))).normalize()), faceVector2);

    let tiltSphereRotated = faceVectorOffsetted.rotate(-simulationAngles.at(2), faceVector2);
    ...
    for (let i = 0; i < 8; i++) {
        qL[i] = (Point.minus(
        		Point.plus(
        			Point.scalarMult(faceVectorLeft, 100),
					Point.scalarMult(faceVector2, 100)
				),
				points[i]
		)).length();
    }
    ...
    for (let i = 0; i < 8; i++) {
        qR[i] = (Point.minus(
            Point.plus(
                Point.scalarMult(faceVectorRight, 100),
                Point.scalarMult(faceVector2, 100)
            ),
            points[i]
        )).length();
    ...
    for (let i = 0; i < 8; i++)
    {
        let vL = clamp(mmap(qL[i] * 2, 250, 400, 1., 0.), 0, 1) / 2;
        let vR = clamp(mmap(qR[i] * 2, 250, 400, 1., 0.), 0, 1) / 2;
...
module.exports.eightChannelsIsotropicAlgorithm = eightChannelsIsotropicAlgorithm;
...
let videojs = __webpack_require__(4);
__webpack_require__(1);
let THREE = __webpack_require__(3);
let $ = __webpack_require__(2);

const eightChannelsAlgorithm = __webpack_require__(0).eightChannelsAlgorithm;
    ...
    for (let side of ['left', 'right'])
    {
        ...
        for (let posH of ['l', 'h'])
        {
            for (let posFB of ['f', 'b'])
            {
                ...
                {
                    audio.push({
                        src: "./audio/" + i + "_" + side[0] + ".mp3", /* ./audio/side_posLRposFBposH.mp3 */
...
$(document).ready(function () {
    let player = window.player = videojs('videojs-panorama-player', {}, function() {
        ...
        this.on('timeupdate', function() {
            if (!(++tic % 100))
            {
                let phase = sound.rewind(player.currentTime());
...
function Sound(audioFiles, player) {
    ...
    function preloadAudio(i) {
        audio[i] = new Audio();
        audio[i].addEventListener('canplaythrough', loadedAudio, false);
        audio[i].src = audioFiles[i].src;
    }
    ...
    {
        for (let i =0; i < volumes.left.length; ++i)
        {
                if (volumes.left[i] < 0 || volumes.left[i] > 1 || volumes.right[i] < 0 || volumes.right[i] > 1)
                {
        ...
        for (let i in audio)
        {
            let bnv = balancedVolumes(volumes.left[i], volumes.right[i]);

            audio[i].volume = bnv.volume;
            audio[i].balance = bnv.balance;
        }
    ...
    function rewind(time) {
        ...
        let now = audio.left[0].currentTime;
        

            for (let i in audio)
            {
                if (audio[i].duration < time) {
                    console.warn('Rewind(out of range):' + time + ' > ' + audioFiles[i].name + '.duration = ' + audio[i].duration);
                    stop();
    ...
    setInterval(function() {
        if (loaded) {
            ...
            document.getElementById("status").innerHTML = "currentTime: <br/>";
                ...
                for (let i in audio)
                {
                    let pos = audio[i].currentTime;
                    if (pos < min) min = pos;
                    if (pos > max) max = pos;

                    document.getElementById("status").innerHTML += audioFiles[i].name + ". " + pos + " , volume: " + audio[i].volume + "<br/>";
                }
