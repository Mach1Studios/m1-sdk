...
Mach1Point3D Mach1Point3D::operator/(float f) {
    return {this->x / f, this->y / f, this->z / f};
}

Mach1Point3D Mach1Point3D::operator-() {
...
Mach1Point3D Mach1Point3D::getCrossed(const Mach1Point3D &b) const {
    return Mach1Point3D(
        y * b.z - z * b.y,
        z * b.x - x * b.z,
        x * b.y - y * b.x);
...
float Mach1Point3D::dot(const Mach1Point3D &vec1, const Mach1Point3D &vec2) {
    return vec1.x * vec2.x + vec1.y * vec2.y + vec1.z * vec2.z;
}

float Mach1Point3D::distance(const Mach1Point3D &vec1, const Mach1Point3D &vec2) {
